          ...
          <div className="bg-zinc-900 border-2 border-primary rounded-xl overflow-hidden">
            {items.map((item) => (
              <div key={item.id} className="flex flex-col sm:flex-row items-center gap-4 p-4 border-b border-zinc-800 last:border-0 hover:bg-white/5 transition-colors">
                <img src={item.image_url} alt={item.name} className="w-24 h-24 object-cover rounded bg-black border border-white/10" />
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center border border-primary rounded bg-black">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3 py-1 text-primary hover:bg-primary hover:text-black transition-colors">-</button>
                    <span className="px-3 py-1 text-white min-w-[40px] text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-3 py-1 text-primary hover:bg-primary hover:text-black transition-colors">+</button>
                  </div>
                  <div className="text-xl font-bold text-primary w-24 text-right">R{(item.price * item.quantity).toFixed(2)}</div>
                  <button onClick={() => removeItem(item.id)} className="text-red-500 hover:text-red-400 p-2 hover:bg-red-500/10 rounded-full transition-colors">
                    <Trash2 size={20} />
                  </button>
                </div>
              </div>
            ...
            {!isCheckingOut ? (
              ...
              <form onSubmit={handleCheckout} className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                <div>
                  ...
                  <input required type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="w-full bg-black border border-zinc-700 focus:border-primary rounded p-2 text-white outline-none transition-colors" placeholder="John Doe" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-primary mb-1 uppercase tracking-wider">Email *</label>
                  <input required type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} className="w-full bg-black border border-zinc-700 focus:border-primary rounded p-2 text-white outline-none transition-colors" placeholder="john@example.com" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-primary mb-1 uppercase tracking-wider">Phone *</label>
                  <input required type="tel" value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="w-full bg-black border border-zinc-700 focus:border-primary rounded p-2 text-white outline-none transition-colors" placeholder="+27 82 123 4567" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-primary mb-1 uppercase tracking-wider">Address *</label>
                  <textarea required value={form.address} onChange={(e) => setForm({...form, address: e.target.value})} className="w-full bg-black border border-zinc-700 focus:border-primary rounded p-2 text-white outline-none transition-colors min-h-[60px]" placeholder="123 Main St..." />
                </div>
                <div>
                  <label className="block text-xs font-bold text-primary mb-1 uppercase tracking-wider">City *</label>
                  <input required type="text" value={form.city} onChange={(e) => setForm({...form, city: e.target.value})} className="w-full bg-black border border-zinc-700 focus:border-primary rounded p-2 text-white outline-none transition-colors" placeholder="Cape Town" />
                </div>
                
                <div className="pt-2 gap-3 flex flex-col">
                  <button type="submit" className="w-full bg-gradient-to-br from-primary to-orange-400 text-black py-3 rounded font-bold hover:scale-[1.02] transition-transform">
                    Place Order
                  </button>
                  <button type="button" onClick={() => setIsCheckingOut(false)} className="w-full bg-transparent border border-zinc-700 text-zinc-400 py-2 rounded hover:text-white hover:border-white transition-colors">
                    Cancel
                  </button>
                </div>
              </form>
