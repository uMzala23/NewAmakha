      ...
      <div className="flex flex-col flex-1">
        {product.review_count > 0 && (
          <div className="flex justify-center items-center gap-1 mb-2 text-primary">
            {[...Array(5)].map((_, i) => (
              <Star key={i} size={16} fill={i < Math.round(product.rating) ? "currentColor" : "none"} className={i < Math.round(product.rating) ? "text-primary" : "text-zinc-600"} />
            ))}
        ...
        <div className="text-2xl font-bold text-primary mb-4 font-serif">R{product.price.toFixed(2)}</div>

        <form onSubmit={handleAddToCart} className="flex gap-2 justify-center items-center mt-auto">
          <input type="number" min="1" max={product.stock} value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="w-16 p-2 bg-black border border-primary text-primary rounded text-center focus:outline-none focus:ring-1 focus:ring-primary" />
          <button type="submit" className="bg-gradient-to-br from-primary to-orange-400 text-black px-6 py-2 rounded font-bold hover:scale-105 transition-transform active:scale-95">
            Add to Cart
          </button>
        </form>
